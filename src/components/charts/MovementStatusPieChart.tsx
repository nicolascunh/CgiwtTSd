...
import { Truck, PauseCircle, AlertTriangle } from "lucide-react"
...
interface MovementStatusPieChartProps {
...
const iconMap: Record<string, any> = {
  'Em movimento': Truck,
  'Parados': PauseCircle,
  'Marcha lenta excessiva': AlertTriangle,
}

const iconColorMap: Record<string, string> = {
  'Em movimento': 'text-green-600',
  'Parados': 'text-blue-600',
  'Marcha lenta excessiva': 'text-orange-600',
}

export function MovementStatusPieChart({
  segments,
  total,
  title = "Status de Movimentação",
  description = "Veículos monitorados",
  statusLabel,
  statusTone = 'info'
}: MovementStatusPieChartProps) {
  ...
  const chartConfig = React.useMemo(() => {
    ...
    segments.forEach(seg => {
      config[seg.label.toLowerCase().replace(/\s+/g, '_')] = {
        label: seg.label,
        color: seg.color,
      }
    })
    ...
    <Card className="flex flex-col">
      ...
      <div className="flex flex-col gap-2 text-sm px-6 pb-4">
        {segments.map((segment) => {
          const Icon = iconMap[segment.label] || PauseCircle
